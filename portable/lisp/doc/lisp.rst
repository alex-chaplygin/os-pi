...
#. (call/cc f). Вызов функции с текущим продолжением. Функция-объект f должна быть с одним параметром возврата, который явялется функцией с одним аргументом - результат вычисления call/cc. При вызове call/cc сохраняется текущее состоние (продолжение). Если внутри функции f вызывается параметр возврата (продолжение через примитив call-continuation), то вычисление возвращается в сохраненное состояние с результатом из параметра.
   ::

      > (call/cc #'(lambda (cont) ;; cont - объект-продолжение
                      1 (call-continuation cont 4) 2 3)) ; вернет 4
...
Для error существует обработчик по умолчанию, который выводит сообщение об ошибке.
	      
Вычисляется выражение, все параметры исключений - списки. Для error пример списка: (division-by-zero).

Выход за границу массива (строки): (index-error <значение индекса> <размер массива>).
       
Исключение может быть внутреннее (внутри примитива) или пользовательское:
::
