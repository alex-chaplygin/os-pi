...
:block: прямоугольная область, может иметь вложенные дочерние элементы, необходимо указывать координаты внутренних элементов
	(block <атрибут1> <атрибут2> ... children <элемент1> <элемент2> ...)
...
Пользовательские атрибуты - это входные параметры элементов, отличающиеся от встроенных элементов, которые можно передавать от родительских элементов к дочерним. 

Пример:
::
	(block (width 200) (height 200) (data1 (1 2 3)) (inner-text "Рост: ") ; атрибуты 
		children (text inner-text) (text data1)) ; дочерние элементы, который используют пользовательский атрибут
...
::
   ...
   (set-screen
       (vert children ; вертикальная группа
	   (text "Test" (id 1)) ; текст
	   (horiz ; горизонтальная группа
	       (block (width 100) (height 100) (color 14) children ; блок элементов
...
:attributes: хеш-таблица пользовательских атрибутов
...
:set-attribute block attr value: установливает в блоке block атрибут attr со значением value
:get-attribute block attr: получить значение атрибута attr блока block

Класс vert
^^^^^^^^^^

Класс vert наследуется от block и служит для группировки базовых элементов, но при этом вместо размещения по координатам положение дочернего элемента вычисляется автоматически так, чтобы дочерние элементы группировались в столбик.
В нем хранятся свойства:
