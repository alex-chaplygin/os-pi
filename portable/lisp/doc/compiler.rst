...
:LABEL lab: Метка с именем lab
:FIX-LET num args expr: let форма, num - число аргументов, args - список выражений аргументов, expr - тело lambda
:FIX-CLOSURE name env body: Замыкание функции (фиксированное число аргументов) с номером окружения env, именем name и телом body.
...
:FIX-CALL name env args: Вызов функции с именем name, номером окружения env и списком выражений аргументов args (фиксированное число аргументов).
:NARY-CALL name num env args: Вызов функции с именем name, номером окружения env и списком выражений аргументов args (переменное число аргументов).
...
:TAIL-NCALL name num env args depth: Хвостовой вызов функции с именем name, номером окружения env, списком выражений аргументов args и смещением в окружении вызова функции от её определения depth (переменное число аргументов).
			      num - число фиксированных аргументов.			      
...
Оптимизация загрузки регистра аккумулятора (удаление неиспользуемого кода)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Если встречается последовательность команд загрузки данных в регистр аккумулятора, то все эти операции, кроме последней удаляются.
::
...
Удаление неиспользуемых функций (удаление неиспользуемого кода)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Удаление функций после подстановки (удаление неиспользуемого кода)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
* замыкание пользовательской функции - FIX-CLOSURE без LABEL (для этого необходимо собрать информацию о замыкании функции в первомпроходе)
...
Заменить примитивы с произвольным числом аргументов на примитивы с двумя аргументами:
::
...
Устранить двойное добавление аргументов в стек
...
Форма setq x e - не хвостовой вызов e, потому что он записывается в переменную.

Форма if e t f. e - не хвостовой вызов.

Форма progn x1 .. xn, все кроме xn - не хвостовые вызовы.

Применение функции - всегда хвостовой вызов для тела функции.
   
Свёртка констант
^^^^^^^^^^^^^^^^

Константные выражения считаются на этапе компиляции:
::

   (setq x (+ 1 2 3)) -> (setq x 6)

Сорачиваются только примитивы не имеющие побочных эффектов, кроме make-array и make-string.

Встраивание функций, которые вызываются один раз (beta-expansion)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Тело функции может также быть оптимизировано, поэтому надо сохранить его.

При подстановке внутри замыканий (FIX-CLOSURE) необходимо учитывать относительный уровень глубины вызова.
::

   (label parse-ihdr ;; функция где происходит подстановка
      (fix-call parse-struct ((const '(1 2 3)))))

   (label parse-struct ;; функция, которая подставляется
      (fix-closure ;; замыкание - относительный уровень вложенности +1
         (fix-call get-struct ((local-ref 0) (deep-ref 1 0) (deep-ref 2 1)))
	 (return)))

   ;; подстановка тела
   (label parse-ihdr
      (fix-closure
         ;; local-ref не меняется, потому что уровень >0 (если = 0, то меняется на выражение аргумента)
	 ;; deep-ref с уровнем 1, = текущему уровню, меняется на выражение аргумента 0
	 ;; (deep-ref 2 1) - понижается уровень на 1
         (fix-call get-struct ((local-ref 0) (const '(1 2 3)) (deep-ref 1 1)))
	 (return))) ;; return нельзя удалить из замыкания
...
:JMPTAIL label: Для машинного кода переход при хвостовом рекурсивном вызове.
...
:FUNC-CALL:      вызывает функцию-замыкание на вершение стека
:CHECK-PRIM:     является ли объект на вершине стека замыканием примитива
:PRIM-CALL:      вызывает функцию-замыкание как примитив на вершение стека
...
:26   FUNC-CALL:      вызывает функцию-замыкание на вершение стека
:27   CHECK-PRIM:     является ли объект на вершине стека замыканием примитива
:28   PRIM-CALL:      вызывает функцию-замыкание как примитив на вершение стека
...
LABEL, которые относятся к функциям компилятор делает как FUNC.
...
Для x86 удобно, когда аргументы генерируются в обратном порядке. В этом случае аргументы уже лежат в стеке в нужном порядке.
...
Сборка мусора для машинного кода
--------------------------------

Сборщик мусора должен пометить используемые объекты, это константы, глобальные переменные, регистр frame_reg, системный стек и стек catch/throw.

Чтобы пометить системный стек в начале run запоминается текущее значение SP как максимальный адрес стека, который нужно помечать.
...
Значение BP в кадре указывает на предыдущий кадр (адрес предыдущего сохраненного BP). Для каждого кадра стека помечаются аргументы до адреса предыдущего BP. Само значение BP и адрес возврата пропускается. В режиме x64, в момент сборки мусора аргументы уже извлечены из стека и находятся в регистрах, поэтому нужно помечать регистры.
