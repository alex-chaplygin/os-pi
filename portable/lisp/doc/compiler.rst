...
Лексер (функция `lexer` в `lexer.lsp`) отвечает за преобразование входной строки в список лексем. Он пропускает пробельные символы между лексемами и использует комбинаторы парсеров для распознавания различных типов лексем.
