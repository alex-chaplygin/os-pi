...
Если функция не рекурсивная, и вызывается один раз, то вместо вызова функции можно подставить скомпилированное тело функции. При этом удаляется объявление функции (LABEL). В теле функции необходимо заменить обращения к переменным:

* локальные переменные (LOCAL-REF) заменяются на скомпилированные выражения аргументов
* установка локальных переменных невозможна (LOCAL-SET), значит встраивание сделать нельзя
* свободные переменные понижаются в уровне обращения (DEEP-REF/SET 2 заменяется на DEEP-REF/SET 1, DEEP-REF/SET 1 заменяется на LOCAL-REF/SET)
