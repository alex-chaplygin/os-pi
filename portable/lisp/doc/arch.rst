...
Глобальное окружение хранится в виде линейного списка символов. Этот список только расширяется или переопределяется. Структура символа хранит в своих полях связанные значения: value - значение переменной, lambda - пользовательская функция, macro - макрос, func - встроенный примитив.
...
Необходимо определить,какие объекты нужно оставить, а какие являются мусором. Выполняем пометку объектов,которые нужно оставить. Помечаются все объекты в глобальном окружении: символы, глобальные переменные, функции, макросы. Помечаются объекты в локальном окружении переменных и функций. Помечаются временные объекты и их привязки промежуточных вычислений. 

Фаза очистки
^^^^^^^^^^^^

Обходим все объекты. Если есть пометка, то пометку снимаем. Если нет - то объект освобождается.

Обработка ошибок и исключений
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Обработчики добавляются в стек обработки исключений. В стек сохраняется окружение на момент вызова handle. При возникновении исключения происходит поиск соответствующего обработчика. Обработчик - это lambda-функция с параметром - список аргументов исключения. При возникновении исключения передается управление (longjmp) на функцию handle, происходит восстановление окружения, ищется и вызывается функция в сохраненном окружении. Если для текущего обработчика буфер восстановления другой, то необходимо вернуться в него (верхний handle). После выхода из handle необходимо восстановить указатель стека исключений. Один и тот же стек используется для исключений и catch, только для catch функция обработки равна nil, только восстанавливается окружение.
