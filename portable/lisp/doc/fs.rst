...
::
   ...
   (write-struct <array> <offset> <struct> <хеш-объект из полей и значений>)
...
Вспомогательная рекурсивная функция хранит позицию bpos и размер записываемого буфера size. Позиция внутри блока - ofs. Длина записываемого фрагмента - len.

Когда мы пишем внутри блока не переходя границу, при рекурсивном вызове размер становится равным 0 и запись заканчивается:
::
...
Когда запись идет с переходом границы блоков, при рекурсивном вызове размер уменьшается по границе, оставшиеся байты будут записаны в следующем вызове:
::
...
Если записываемый блок с переходом границы - последний, то необходимо перед рекурсивным вызовом добавить новый блок, оставшиеся байты будут записаны в следующем вызове.
размер уменьшается по границе.

Размер файла увеличивается если position + len > size.
