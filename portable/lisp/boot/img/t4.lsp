...
(defun t4-run (huff val)
  (&&& n->(huff-decode huff)
       return (if (eq n 'EOL) nil (make-list n val))))

(defun t4-line ()
  (parse-app (parse-many (&&& w->(t4-run *white-run* 1)
			      b->(parse-optional (t4-run *black-run* 0))
			      return (append w b))) #'flatten))
...
(defun parse-t4 ()
  (parse-app (parse-many (&&& (parse-EOL) t4->(t4-line) return t4)) #'flatten))
