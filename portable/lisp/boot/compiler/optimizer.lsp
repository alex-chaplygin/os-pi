  ...
  (cond ((null exp) nil)
	...
	((and (pairp exp) (atom (car exp)))
	 (case (car exp)
	   ...
	   ('FUNC (let* ((f (get-hash *functions-info* (second exp)))
			 (dc (if (check-key f 'decl-closure) (get-hash f 'decl-closure) 0)))
		    (set-hash f 'decl-closure (++ dc))
		    (if (< dc 2) (list 'FUNC (second exp) (beta-exp (third exp) args level)) (list 'NOP))))
...
(defun optimize-tree1 (tree)
  ...
  (labels ((optimize-many (tree) (map #'optimize tree))
	   ...
	   (optimize (tree)
	     (if (contains '(NOP CONST GLOBAL-REF LOCAL-REF DEEP-REF RETURN PRIM-CLOSURE NPRIM-CLOSURE GOTO LABEL) (car tree))
		 tree
		 (case (car tree)
...
(defun optimize-tree2 (tree)
  ...
  (cond ((null tree) tree)
	...
	((and (pairp tree) (cdr tree))
	   (case (car tree)
	     ('FIX-CLOSURE (if (and (cdddr tree) (forth tree) (pairp (forth tree))) (list 'FIX-CLOSURE (second tree) (third tree) (if (null (forth tree)) nil (list (car (forth tree)) (second tree) (optimize-tree2 (third (forth tree)))))) tree))
	     ('FUNC (let* ((f (get-hash *functions-info* (second tree)))
			   (l (list (car tree) (second tree) (optimize-tree2 (third tree))))
			   (dc (-- (get-hash f 'decl-count))))
		      (set-hash f 'body (third l))
		      (set-hash f 'decl-count dc)
		      (if (or (and (contains *optimize-flags* 'dead-code-elimination)
