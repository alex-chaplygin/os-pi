...
(deftest lex-test-badfloat ()
  "Тест: Некорректное число с плавающей точкой"
  (print (assert (handle (lisp-lexer "-.") (parse-error (x) x)) '((1 1)"lisp-lexer: got dot with sign")))
  (print (assert (handle (lisp-lexer "-2.A5") (parse-error (x) x)) '((1 1)"lisp-lexer: invalid float number"))))


(deftest lex-test-more-errors ()
  "Тест: Дополнительные ошибки лексера" 
  (print (assert (handle (lisp-lexer "\"\\") (parse-error (x) x)) '((1 3)"lisp-lexer: unexpected end of escape sequence")))
  (print (assert (handle (lisp-lexer "\"\\a\"") (parse-error (x) x)) '((1 3)"lisp-lexer: unexpected end of escape sequence"))))
