...
(defun test-match (str regex)
  ...
  (print str regex)
  (if (regex-match (compile-regex regex) str) (match str regex) NIL))
...
(deftest test-regex ()
  ...
  (print (assert (test-match "a" ".") "a"))
  (print (assert (test-match "" ".") NIL))
  (print (assert (test-match "aaaatd" "a*t") "aaaat"))
  (print (assert (test-match "aaaat" "a*t") 't))
  (print (assert (test-match "etd" "a*t") "t"))
  (print (assert (test-match "test" "t.*t") "test"))
  (print (assert (test-match "test2" "t.*es.*t2") "test2"))
  (print (assert (test-match "string" ".*") "string"))
  (print (assert (test-match "string" ".*") ""))
  ...
  (print (assert (test-match "abcktrrrq" ".+ktr+") "abcktrrr"))
  (print (assert (test-match "abcktrrrq" ".+ktr+") "abcktr"))
  (print (assert (test-match "Hello" "Hel+o") "Hello"))
  (print (assert (test-match "heo" "hel+o") NIL))
  (print (assert (test-match "Helo" "Hel?lo") "Helo"))
  (print (assert (test-match "привет" "при.*т") "привет")) ; ограниченная поддержка юникода (рассматривает русские буквы как 2 байта символов)
