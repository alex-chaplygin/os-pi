(deftest close-file-test () "close file test"
         "Проверка закрытия файла"
         (let ((file (make-file "test" 512 '(0 . 0) '(6))))
           (close-file file)
           ;;(assert file '(()))
           (assert file #(0 0 0 0 0))))

(deftest tell-file-test () "tell file test"
         "Проверка получения позиции в файле"
         (let ((file (make-file "test" 512 '(4 . 12) '(6))))
           (assert (tell-file file) 2060)))

(deftest seek-file-test1 () "seek file test1"
         "Проверка установки позиции в файле от начала"
         (let ((file (make-file "test" 1024 '(0 . 0) '(6 7))))
           (seek-file file 524 'SET)
           (assert (File-position file) '(1 . 12))))

(deftest seek-file-test2 () "seek file test2"
         "Проверка установки позиции в файле от текущей позиции с переходом между блоками"
         (let ((file (make-file "test" 1024 '(0 . 12) '(6 7))))
           (seek-file file 512 'CUR)
           (assert (File-position file) '(1 . 12))))

(deftest seek-file-test2-2 () "seek file test2.2"
         "Проверка установки позиции в файле от текущей позиции без переходов между блоками"
         (let ((file (make-file "test" 512 '(0 . 12) '(6))))
           (seek-file file 400 'CUR)
           (assert (File-position file) '(0 . 412))))

(deftest seek-file-test3 () "seek file test3"
...
(deftest read-file-test1 () "read file test1"
         "Проверка на чтении файла с начала файла"
         (let ((test-sector (make-array 512))
               (file (make-file "test" 512 '(0 . 0) '(10)))
               (file-buf nil))
           (for i 0 512
...
(deftest read-file-test2 () "read file test2"
         "Проверка на чтении файла не с начала файла"
         (let ((test-sector (make-array 512))
               (file (make-file "test" 512 '(0 . 120) '(11)))
               (file-buf nil))
           (for i 0 512
                (seta test-sector i (& i 0xff)))
           (set-block-size 1)
...
(deftest read-file-test3 () "read file test3"
         "Проверка на чтении файла не с начала файла с переходом между блоками"
         (let ((test-sector (make-array 512))
               (file (make-file "test" 1024 '(0 . 1) '(12 13)))
               (file-buf nil))
           (for i 0 512
                (seta test-sector i (& i 0xff)))
           (set-block-size 1)
...
;; (let ((test-sector (make-array 512))
;;       (file (make-file "test" 512 '(0 . 1) '(11)))
;;       (file-buf nil))
;;   (for i 0 512
;;        (seta test-sector i (& i 0xff)))
;;   (set-block-size 1)
...
(deftest write-file-test1 () "write file test1"
         "Проверка на запись в файл с начала файла"
         (let ((test-sector (make-array 512))
               (file (make-file "test" 512 '(0 . 0) '(14)))
               (file-buf nil))
           (for i 0 512
                (seta test-sector i (& i 0xff)))
           (set-block-size 1)
...
(deftest write-file-test3 () "write file test3"
         "Проверка на запись в файл не с начала файла c переходом между блоками"
         (let ((test-sector1 (make-array 512))
               (test-sector2 (make-array 512))
               (file (make-file "test" 1024 '(0 . 511) '(14 15)))
               (file-buf nil))
           (for i 0 512
                (seta test-sector1 i (& i 0xff))
                (seta test-sector2 i (& 99 0xff)))
...
(deftest write-file-test4 () "write file test4"
