  ...
  (&&& bpb-struct-> (parse-struct '((jmp-command . 3)
       ...
       return (progn
		(unless (= 512 (arr-get-num (get-hash bpb-struct 'bytes-per-sector) 0 2))
                  (raise 'error "parse-bios-parameter-block: bytes per sector in bpb is not 512"))
		(let ((hidden-sectors-count (arr-get-num (get-hash bpb-struct 'hidden-sectors-count) 0 4)))
			...
			(active-fat-struct (arr-get-num (get-hash bpb-struct 'active-fat-struct) 0 2)))
                    ...
                    (FAT32FileSystem-set-fat-sectors *file-system* (+ hidden-sectors-count (arr-get-num (get-hash bpb-struct 'sectors-per-fat-fat32) 0 4)))
                    (for i 0 fat-tables-count
                         (seta (FAT32FileSystem-fat-start-sectors *file-system*) i (+ hidden-sectors-count (+ (arr-get-num (get-hash bpb-struct 'reserved-sectors-count) 0 2) (* i (FAT32FileSystem-fat-sectors *file-system*))))))
			...
			(FAT32FileSystem-set-volume-size *file-system* volume-size-small)))
                  ...
                  (FAT32FileSystem-set-fsinfo-sector *file-system* (+ hidden-sectors-count (arr-get-num (get-hash bpb-struct 'fsinfo-sector) 0 2)))
...
(defmethod fs-init ((fat32fs FAT32FileSystem) disk start-sector sector-count)
  ...
  (when (> (FAT32FileSystem-volume-size fat32fs) sector-count) (raise 'error "fs-init(fat32): volume size in bpb is higher than volume size in args"))
  ...
  (setq *root-directory* (FAT32FileSystem-root-entry *file-system*))
          ...
          (while (not (null block-stream))
            (let ((next-byte (get-byte block-stream)))
              (unless (null next-byte)
                (case (car next-byte)
                  (+fat32-entry-deleted+ (setq block-stream (cdr (get-array block-stream 32))))
                     ...
                     (if (= entry-attributes +fat32-lfn-attribute+)
                         ...
                         (progn
                           (let ((parse-res (funcall (parse-fat32-dir-entry) block-stream)))
                             ...
                             (unless (equal lfn-name "")
                               (set-hash (car parse-res) 'name lfn-name))
                             (setq lfn-name "")
                             (unless (fat32-is-special-name (get-hash (car parse-res) 'name))
...
(defmethod remove-dir* ((fat32fs FAT32FileSystem) parent-dir dir)
  ...
  (when (null (get-hash dir 'dir))
    (raise 'not-directory "remove-dir: entry is not a dir"))
  (when (not (null (car (get-hash dir 'dir))))
    (raise 'not-empty-dir "remove-dir: dir is not empty"))
...
(defmethod free-space* ((fat32fs FAT32FileSystem))
  ...
  (let ((free-count (FAT32FileSystem-free-blocks-count fat32fs)))
    ...
    (* free-count *block-size*)))

(defmethod create-file* ((fat32fs FAT32FileSystem) dir name)
  "Создать файл с названием name в каталоге dir"
  (when (fat32-is-name-duplicate dir name)
    (raise 'file-exists "create-file: file exists"))
  (let ((new-entry (make-hash))
    ...
    (set-hash new-entry 'access-date (list (nth date-time 0) (nth date-time 1) (nth date-time 2)))
...
(defmethod create-dir* ((fat32fs FAT32FileSystem) dir name)
  ...
  (when (fat32-is-name-duplicate dir name)
    (raise 'file-exists "create-file: directory exists"))
  (let ((new-entry (make-hash))
        (date-time (get-cur-time)))
    (set-hash new-entry 'name (copy-tree name))
    (set-hash new-entry 'size 0)
    (set-hash new-entry 'first-block ())
    (new-block fat32fs new-entry)
    (set-hash new-entry 'creation-date-time (append date-time (list 0)))
    ...
    (set-hash dir 'dir
              (list (append (car (get-hash dir 'dir)) (list new-entry))))
    (fat32-create-special-entries new-entry)
    new-entry))

(defmethod set-attr* ((fat32fs FAT32FileSystem) entry new-attributes)
  "Изменить атрибуты файла entry на new-attributes"
  (let ((attributes (fat32-sort-attributes new-attributes (not (null (get-hash entry 'dir))))))
    (fat32-update-entry entry `((attributes . ,attributes)))))
