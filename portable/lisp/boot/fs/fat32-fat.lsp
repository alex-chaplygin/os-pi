...
(defconst +fat-sector-elems+ (/ +sector-size+ +fat-elem-size+)) ;; количество элементов таблицы FAT в секторе(512 байт)
...
(defun fat-read-sector (fat-table sector-num)
  "Прочитать сектор sector-num таблицы FAT, записав содержимое сектора таблицы в массив fat-table"
  (let ((cur-table-elem (* sector-num +fat-sector-elems+))
...
(defun get-fat-chain (block-num)
  ...
  (let ((fat-hash (FAT32FileSystem-fat-table *file-system*))
        (fat-table nil)
        (fat-read-sectors nil)
        (cur-elem nil)
        (prev-elem nil)
        (fat-chain nil))
    (when (>= block-num (* (FAT32FileSystem-fat-sectors *file-system*) +fat-sector-elems+))
      (raise 'error (concat (concat (concat (concat "get-fat-chain: block-num out of range: '" (inttostr block-num)) "' where maximum is: '") (inttostr (-- (* (FAT32FileSystem-fat-sectors *file-system*) +fat-sector-elems+))) "'"))))
    (if (check-key fat-hash block-num) (cons block-num (get-hash fat-hash block-num))
        (progn
          ...
          (setq fat-read-sectors (make-array (FAT32FileSystem-fat-sectors *file-system*)))
          (for i 0 (array-size fat-read-sectors) (seta fat-read-sectors i nil))
          (fat-read-sector fat-table (car (fat-elem-pos block-num)))
          (seta fat-read-sectors (car (fat-elem-pos block-num)) t)
          ...
          (while (!= cur-elem +fat-block-end+)
            (when (and (< cur-elem +fat-block-damaged+) (>= cur-elem (* (FAT32FileSystem-fat-sectors *file-system*) (/ +sector-size+ +fat-elem-size+))))
              (raise 'error "get-fat-chain: elem in fat-chain is out of range"))
            (when (= cur-elem +fat-block-free+)
              (raise 'error (concat "get-fat-chain: fat table elem in chain is free on block num = " (inttostr prev-elem))))
...
(defun get-free-block (fat-table free-block-num)
  ...
  (let ((free-num nil))
    ...
    (when (null free-num)
      ...
      (when (null free-num) (raise 'not-enough-space "get-free-block: free block not found")))
    free-num))
