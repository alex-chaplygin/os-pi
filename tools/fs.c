 ...
 * @brief Консольная утилита fs предназначена для создания
 * виртуального диска и добавления на него файлов. Пример
 * создания виртуального диска:
 * 
 * fs create -o disk -s 1024
 * 
 * Данная команда создаёт виртуальный диск disk размером
 * 1024 блока. Для записи файла на диск необходимо написать
 * следующее:
 * 
 * fs write -d disk -f file.txt
 * 
 * Данная команда записывает на виртуальный диск disk файл
 * file.txt. Каждый файл на виртуальном диске будет занимать
 * целое число блоков.
 * @version 0.1
 * @date 2020-11-08
 * 
